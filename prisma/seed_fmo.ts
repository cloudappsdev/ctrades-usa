...
function buildContractors(now: Date) {
  ...
  for (let i = 0; i < COUNT; i += 1) {
    ...
    rows.push({
      ...
      // Uniformly spread across the last seven days.
      mostRecentlyActive: new Date(now.getTime() - random() * WEEK_MS),
    });
  }
...
/** `tsx prisma/seed.ts --dry-run` prints a sample without touching the database. */
function dryRun() {
